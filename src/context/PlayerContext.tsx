...
export const PlayerProvider = ({ children }: any) => {
    ...
    // ── Playlist — filters erotic based on current settings ───────────────────
    const loadPlaylist = async (): Promise<any[]> => {
        try {
            ...
            const enriched = stories
                ...
                .filter((s: any) => {
                    if (s.isErotic !== 'true') return true;             // always include non-erotic
                    if (!eroticEnabledRef.current) return false;         // erotic disabled: exclude
                    if (!eroticInPlaylistRef.current) return false;      // erotic not in playlist: exclude
                    return true;
                });
